...
export default function App() {
  ...
  useEffect(() => {
    ...
    if (isDefault && !backupIsDefault) {
      ...
      handleRestoreBackup(latestBackup, true);
    }
  ...
  useEffect(() => {
    ...
    const unsubMilitares = onSnapshot(collection(db, 'militares'), (snap) => {
      ...
      let hasCustomLocalMil = false;
      let localMilList: Militar[] = [];
      if (savedLocalMilStr) {
        try {
          localMilList = JSON.parse(savedLocalMilStr);
          if (localMilList && localMilList.length > 0 && !isRosterDefault(localMilList)) {
            hasCustomLocalMil = true;
          }
        } catch (e) {}
      }

      if (snap.empty || isRosterDefault(list)) {
        if (hasCustomLocalMil) {
          console.log("Detectado efetivo customizado localmente. Enviando para o Firestore...", localMilList.length);
          localMilList.forEach(async (m) => {
            try {
              await setDoc(doc(db, 'militares', m.id), sanitizeForFirestore(m));
            } catch (e) {
              console.error("Erro ao sincronizar militar local para o cloud:", e);
            }
          });
    ...
    const unsubEscalas = onSnapshot(collection(db, 'escalas'), (snap) => {
      ...
      let hasCustomLocalEsc = false;
      let localEscList: Escala[] = [];
      if (savedLocalEscStr) {
        try {
          localEscList = JSON.parse(savedLocalEscStr);
          const isDefault = localEscList.length === ESCALAS_INICIAIS.length &&
                            localEscList.every(e => ESCALAS_INICIAIS.some(de => de.id === e.id));
          if (localEscList && localEscList.length > 0 && !isDefault) {
            hasCustomLocalEsc = true;
          }
        } catch (e) {}
      }

      const firestoreIsDefault = list.length === ESCALAS_INICIAIS.length &&
                                 list.every(e => ESCALAS_INICIAIS.some(de => de.id === e.id));

      if (snap.empty || firestoreIsDefault) {
        if (hasCustomLocalEsc) {
          console.log("Detectadas escalas customizadas localmente. Enviando para o Firestore...", localEscList.length);
          localEscList.forEach(async (e) => {
            try {
              await setDoc(doc(db, 'escalas', e.id), sanitizeForFirestore(e));
            } catch (err) {
              console.error("Erro ao sincronizar escala local para o cloud:", err);
            }
          });
    ...
    const unsubPermutas = onSnapshot(collection(db, 'permutas'), (snap) => {
      ...
      let hasCustomLocalPerm = false;
      let localPermList: Permuta[] = [];
      if (savedLocalPermStr) {
        try {
          localPermList = JSON.parse(savedLocalPermStr);
          const isDefault = localPermList.length === PERMUTAS_INICIAIS.length &&
                            localPermList.every(p => PERMUTAS_INICIAIS.some(dp => dp.id === p.id));
          if (localPermList && localPermList.length > 0 && !isDefault) {
            hasCustomLocalPerm = true;
          }
        } catch (e) {}
      }

      const firestoreIsDefault = list.length === PERMUTAS_INICIAIS.length &&
                                 list.every(p => PERMUTAS_INICIAIS.some(dp => dp.id === p.id));

      if (snap.empty || firestoreIsDefault) {
        if (hasCustomLocalPerm) {
          console.log("Detectadas permutas customizadas localmente. Enviando para o Firestore...", localPermList.length);
          localPermList.forEach(async (p) => {
            try {
              await setDoc(doc(db, 'permutas', p.id), sanitizeForFirestore(p));
            } catch (err) {
              console.error("Erro ao sincronizar permuta local para o cloud:", err);
            }
          });
          setPermutas(localPermList);
          return;
        } else if (snap.empty) {
          console.log("Seeding default permutas into empty Firestore collection...");
          PERMUTAS_INICIAIS.forEach(async (p) => {
            try {
              await setDoc(doc(db, 'permutas', p.id), sanitizeForFirestore(p));
            } catch (err) {
              console.error("Error auto-seeding permuta:", err);
            }
          });
          return;
        }
  ...
  const handleRestoreBackup = async (snapshot: BackupSnapshot, silent = false) => {
    try {
